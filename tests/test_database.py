...
def test_filter_podcasts(db):
    ...
    mock_table.filter.assert_called_with("average_rating IS NOT NULL AND scraped_at IS NOT NULL")
...
def test_join_and_select(db):
    ...
    # Call the method
    db.join_and_select(
        "categories", "podcasts", "podcast_id",
        ["podcast_id", "category"], ["podcast_id", "slug"],
        min_filter=1, max_filter=5, min_date="2019-07-07", max_date="2019-07-07"
    )
...
def test_add_composed_column(db):
    ...
    mock_project.assert_called_once_with("*, COALESCE(category, '') || ' ' || COALESCE(slug, '') AS full_info")
